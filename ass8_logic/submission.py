...
def formula2d():
    ...
    Mother = lambda x, y: And(Female(y), Parent(x, y))
    # Father should have been used instead of Parent in Exists below to account
    # not for maternal gradmother.
    # Father = lambda x, y: And(Not(Female(y)), Parent(x, y))
    x, y, z = '$x', '$y', '$z'
    return Forall(x, Forall(y,
        Equiv(
            Grandmother(x, y),
            Exists(z, And(Parent(x, z), Mother(z, y))))))
    # END_YOUR_CODE
...
def ints():
    ...
    formulas.append(Forall('$x', Or(And(Odd('$x'), Not(Even('$x'))), And(Not(Odd('$x')), Even('$x')))))
...
def createRule3():
    ...
    return GrammarRule('$Clause', ['if', 'a', '$Noun', '$Verb', 'a', '$Noun', 'then', 'the', 'former', '$Verb', 'the', 'latter'],
        lambda args: Forall('$x', Forall('$y', Implies(And(And(Atom(args[0].title(), '$x'), Atom(args[2].title(), '$y')), Atom(args[1].title(), '$x', '$y')),
