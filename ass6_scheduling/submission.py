...
def create_chain_csp(n):
    ...
    for i in range(len(variables)-1):
        csp.add_binary_factor(variables[i], variables[i+1], lambda d1, d2: d1^d2)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for v1 in variables:
        for v2 in variables:
            ...
            csp.add_binary_factor(v1, v2, lambda v1, v2: v1[0] + v1[1] != v2[0] + v2[1])
            csp.add_binary_factor(v1, v2, lambda v1, v2: v1[0] - v1[1] != v2[0] - v2[1])
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (our solution is 7 lines of code, but don't worry if you deviate from this)
            def getConsistentCount(bs, assignment, var):
                return len([1 for val in bs.domains[var] if bs.get_delta_weight(assignment, var, val) != 0])

            domain_cnts = [(getConsistentCount(self, assignment, var), i, var) 
                    for i, var in enumerate(self.csp.variables)
                    if var not in assignment]
            return min(domain_cnts)[2]
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        while len(q) != 0:
            var1, q = q[0], q[1:]
            # updated_domain = [val1 for val1 in self.domains[var1] if self.csp.unaryFactors.get(var1, {}).get(val1, 0) != 0]
            # self.domains[var1] = updated_domain
            for var2 in self.csp.binaryFactors[var1].keys():
                consistent_vals = [val2 for val2 in self.domains[var2] if isConsistent(var1, var2, val2)]
                if len(consistent_vals) != len(self.domains[var2]):
                    self.domains[var2] = consistent_vals
                    q.append(var2)
        # END_YOUR_CODE
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    last_sum = ('sum', name, ())
    csp.add_variable(last_sum, [(0, 0),])

    sum_vals = list(range(0, maxSum + 1))
    domain = [(v1, v2) for v1 in sum_vals for v2 in sum_vals]

    for i in range(len(variables)):
        y = ('sum', name, last_sum[2] + (variables[i],))
        csp.add_variable(y, domain)
        csp.add_binary_factor(y, last_sum, lambda yv, last_sumv: yv[0]  == last_sumv[1])
        csp.add_binary_factor(y, variables[i], lambda yv, varv: yv[0] + varv == yv[1])
        last_sum = y
    rname = ('result', name, tuple(variables))
    csp.add_variable(rname, sum_vals)
    csp.add_binary_factor(rname, last_sum, lambda r, ls: r == ls[1])
    return rname
    # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for cid in courses:
            for quarter in self.profile.quarters:
                ...
                csp.add_variable((cid, quarter), list(range(course.minUnits, course.maxUnits+1)) + [0])
        for req in self.profile.requests:
            for cid in req.cids:
                for quarter in self.profile.quarters:
                    cid_local = cid
                    csp.add_binary_factor((req, quarter), (cid, quarter), \
                        lambda rq, cq: (rq == cid_local and cq !=0) or (rq != cid_local and cq == 0))
        for quarter in self.profile.quarters:
            credits = [(cid, quarter) for cid in courses]
            credit_sum = get_sum_variable(csp, quarter, credits, self.profile.maxUnits)
            csp.add_unary_factor(credit_sum, lambda s: s >=self.profile.minUnits and s <= self.profile.maxUnits)
