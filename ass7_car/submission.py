...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for r in range(self.belief.numRows):
            for c in range(self.belief.numCols):
                cy = util.rowToY(r)
                cx = util.colToX(c)
                dist = math.sqrt(math.pow(agentX-cx, 2) + math.pow(agentY-cy, 2))
    ...
    def elapseTime(self):
        ...
        nr = self.belief.numRows
        nc = self.belief.numCols
        p=[[0]*nc for _ in range(nr)]
        tp = {}
        for item in self.transProb.keys():
            if item[0] not in tp:
                tp[item[0]] = []
            tp[item[0]].append(item[1])
        for r1 in range(nr):
            for c1 in range(nc):
                sourceProb = self.belief.getProb(r1, c1)
                for transition in tp.get((r1, c1), []):
                    r2, c2 = transition
                    p[r2][c2] +=  sourceProb * self.transProb[((r1, c1), (r2, c2))]
        for r1 in range(nr):
            for c1 in range(nc):
                self.belief.setProb(r1, c1, p[r1][c1])
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        nps = {}
        num_parts = 0
        for (r, c), cnt in self.particles.items():
            num_parts += cnt
            cy = util.rowToY(r)
            cx = util.colToX(c)
            dist = math.sqrt(math.pow(agentX-cx, 2) + math.pow(agentY-cy, 2))
            p = util.pdf(dist, Const.SONAR_STD, observedDist)
            nps[(r, c)] = cnt * p
        self.particles = collections.Counter(util.weightedRandomChoice(nps) for _ in range(num_parts))
        # END_YOUR_CODE
    ...
    def elapseTime(self):
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        self.particles, particles = collections.defaultdict(int), self.particles
        for cur_pos, cnt in particles.items():
            if cur_pos in self.transProbDict:
                for _ in range(cnt):
                    self.particles[util.weightedRandomChoice(self.transProbDict[cur_pos])] += 1
        # END_YOUR_CODE
